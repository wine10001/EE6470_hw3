...
void Testbench::feed_rgb() {
  ...
  unsigned char R, G, B, flag;      // color of R, G, B and flag
...
void Testbench::fetch_result() {
  ...
  for (y = 0; y != height; ++y) {
    for (x = 0; x != width; ++x) {
      ...
      total_latency += (double)latency_time.to_seconds() * 100000000;       //  clk/output
#else
			total = i_result.read();
#endif
      //total_count += 1;
			*(target_bitmap + bytes_per_pixel * (width * y + x) + 2) = total.range(7, 0);
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 1) = total.range(15, 8);
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 0) = total.range(23, 16);
    }
  }
